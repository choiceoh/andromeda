...
//
// Output is React elements so every text node is auto-escaped — no HTML
// sanitizer needed, and link/image schemes are whitelisted. The ONE exception is
// KaTeX math: its `renderToString` HTML is injected via dangerouslySetInnerHTML.
// That is safe because the HTML is generated by KaTeX from the LaTeX source (not
// passed-through user HTML), and `trust` defaults to false so `\href`/`\url` and
// other URL-injecting macros are disabled — only KaTeX's own span/MathML markup
// is produced. We render our own GFM rather than pulling react-markdown + remark
// (and its build-script supply-chain gate); KaTeX is the single math dependency.
